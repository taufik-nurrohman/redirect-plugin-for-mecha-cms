...
<div class="tab-content-area">
  <div class="tab-content" id="tab-content-1-1">
    <?php if($chunks): ?>
    <table class="table-bordered table-full-width table-redirect" id="table-redirect" data-confirm-delete-text="<?php echo $speak->notify_confirm_delete; ?>">
      <thead>
        <tr>
  ...
  <div class="tab-content hidden" id="tab-content-1-3">
    <form class="form-plugin" action="<?php echo $config->url . '/' . $config->manager->slug; ?>/plugin/<?php echo File::B(__DIR__); ?>/update" method="post">
      ...
      <label class="grid-group">
        <span class="grid span-1 form-label"><?php echo $speak->plugin_redirect_title_domain . ' <span class="text-info help" title="' . $speak->plugin_redirect_description_domain . '">' . Jot::icon('question-circle') . '</span>'; ?></span>
